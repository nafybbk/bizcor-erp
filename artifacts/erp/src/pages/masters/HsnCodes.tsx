...
export default function HsnCodes() {
  ...
  // Items whose saved HSN code (typed before this searchable master existed)
  // doesn't match anything in the official list — GST filing only accepts
  // government-approved codes, so these need to be fixed or cleared.
  const [invalidItems, setInvalidItems] = useState<any[]>([]);
  const [invalidLoading, setInvalidLoading] = useState(true);
  const [invalidEdits, setInvalidEdits] = useState<Record<number, string>>({});
  const [invalidSavingId, setInvalidSavingId] = useState<number | null>(null);
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      api.get<any>("/items?limit=9999").then(r => r.data).catch(() => []),
    ]).then(([mine, dir, allItems]) => {
      ...
      const officialCodes = new Set<string>([...dirCodes, ...(mine as any[]).map((c: any) => String(c.code).trim())]);
      const bad = (allItems as any[]).filter(it => {
        const code = String(it.hsnCode || "").trim();
        return code && !officialCodes.has(code);
      });
      setInvalidItems(bad);
  ...
  const saveInvalidItemHsn = async (itemId: number, newCode: string) => {
    setInvalidSavingId(itemId);
    try {
      await api.patch(`/items/${itemId}`, { hsnCode: newCode.trim() || null });
      load();
    } finally { setInvalidSavingId(null); }
  };
  ...
  return (
    <div className="max-w-4xl space-y-4">
      ...
      {!invalidLoading && invalidItems.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl overflow-hidden">
          <button
            ...
            <div className="flex items-center gap-2 text-sm font-semibold text-red-800">
              ...
              {invalidItems.length} item{invalidItems.length > 1 ? "s" : ""} have an HSN code not in the official list — GST filing will reject these
            </div>
            <ChevronDown className={`w-4 h-4 text-red-500 transition-transform ${showInvalid ? "rotate-180" : ""}`} />
          </button>
          {showInvalid && (
            <div className="border-t border-red-100 divide-y divide-red-100 bg-white">
              {invalidItems.map((it: any) => (
                <div key={it.id} className="flex items-center gap-3 px-4 py-2.5">
                  ...
                  <span className="font-mono text-xs font-bold text-red-600 bg-red-50 px-2 py-1 rounded">{it.hsnCode}</span>
                  <input
                    className={inputCls + " w-32 text-sm"}
                    placeholder="Correct HSN"
                    value={invalidEdits[it.id] ?? ""}
                    onChange={e => setInvalidEdits(m => ({ ...m, [it.id]: e.target.value }))}
                  />
                  <button
                    disabled={invalidSavingId === it.id || !(invalidEdits[it.id] ?? "").trim()}
                    onClick={() => saveInvalidItemHsn(it.id, invalidEdits[it.id] ?? "")}
                    className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded-lg disabled:opacity-40">
                    {invalidSavingId === it.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : "Save"}
                  </button>
                  <button
                    disabled={invalidSavingId === it.id}
                    onClick={() => saveInvalidItemHsn(it.id, "")}
                    title="Clear this item's HSN code"
                    className="px-2 py-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg">
