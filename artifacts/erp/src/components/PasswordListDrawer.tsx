...
function PasswordCell({ pwd, id }: { pwd?: string; id: string }) {
  const [show, setShow] = useState(false);
  const [copied, setCopied] = useState(false);

  if (!pwd) return <span className="text-gray-300 text-xs italic">set nahi</span>;
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 text-xs text-gray-500 uppercase tracking-wide">
                  <tr>
                    ...
                    <th className="px-4 py-2.5 text-left">Phone / Email</th>
                    ...
                    superAdmins.map((a) => (
                      <tr key={a.id} className="hover:bg-yellow-50/40">
                        ...
                        <td className="px-4 py-3">
                          <PasswordCell pwd={a.plain_password} id={`sa_${a.id}`} />
                        </td>
                      </tr>
                    ))
                  )}
                ...
                <tbody className="divide-y divide-gray-100">
                  {users.length === 0 ? (
                    ...
                    users.map((u) => (
                      <tr
                        key={u.id}
                        className={`hover:bg-blue-50/30 ${!u.is_active ? "opacity-40" : ""}`}
                      >
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <PasswordCell pwd={u.plain_password} id={`u_${u.id}`} />
                        </td>
                      </tr>
                    ))
                  )}
